            ...
            <AnimatePresence>
              {filteredEvents.map(event => (
                <motion.div 
                  ...
                  whileHover={{ y: -6 }}
                  key={event.id} 
                  className="relative bg-background/40 backdrop-blur-2xl border border-border/30 rounded-2xl overflow-hidden hover:border-accent/60 transition-all duration-500 group flex flex-col shadow-lg hover:shadow-[0_8px_40px_rgba(59,130,246,0.2)]"
                >
                  {/* Neon Glow Behind Card on Hover */}
                  <div className="absolute inset-0 bg-gradient-to-br from-accent/20 to-blue-500/0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-2xl pointer-events-none" />
                  ...
                  <div className="p-6 border-b border-border/30 relative z-10">
                    ...
                    <span className={`absolute top-5 right-5 px-2.5 py-1 text-[10px] uppercase font-black tracking-widest rounded-lg border shadow-sm ${
                      event.status === 'Active' 
                        ? 'bg-green-500/10 text-green-400 border-green-500/30 shadow-[0_0_10px_rgba(34,197,94,0.3)]' :
                  ...
                  <div className="p-6 grid grid-cols-2 gap-y-4 gap-x-4 text-sm flex-1 relative z-10">
                    <div className="bg-background/30 p-3 rounded-xl border border-border/20 group-hover:bg-background/50 transition-colors">
                      <span className="text-muted-foreground/80 text-[11px] font-bold uppercase tracking-wider block mb-1">Participants</span>
                      <div className="flex items-center gap-2 font-bold text-lg">
                        <Users className="w-4 h-4 text-blue-400" /> {event.participantsCount}
                      </div>
                    </div>
                    <div className="bg-background/30 p-3 rounded-xl border border-border/20 group-hover:bg-background/50 transition-colors">
                      <span className="text-muted-foreground/80 text-[11px] font-bold uppercase tracking-wider block mb-1">Certificates</span>
                      <div className="flex items-center gap-2 font-bold text-lg">
